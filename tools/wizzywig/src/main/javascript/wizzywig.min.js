...
function WizzyWig(a,b){function d(a){a.hasClass("editable")||(a.addClass("editable"),a.click(Inputter.acceptInputFor(a)))}$(b).each(function(){d($(this))});var c=new DragonController(d);c.makeMoveable($(WizzyWig.draggableElements));c.makeDropTarget($(WizzyWig.draggableElements));$("body").append(WizzyWig.console);$("#wizzywigConsole .save").click(function(b){a.save(b)});c.makeCopyable($("#wizzywigConsole table"))}WizzyWig.editableElements="td, h1, h2, p";WizzyWig.draggableElements="table, h1, h2, p";
...
WizzyWig.console='<div class="middleFixed"><div id="wizzywigConsole"><span id="plus">+</span><div class="left spaced" style="margin-top: 20px; ">'+WizzyWig.saveButton+WizzyWig.helpButton+"</div>"+WizzyWig.simpleConcept+WizzyWig.multiRowConcept+WizzyWig.clearDiv+"</div></div>";WizzyWig.knownBugs="These features are known to misbehave in certain browsers:\n * Input fields don't respond to mouse clicks (Firefox)\n";
